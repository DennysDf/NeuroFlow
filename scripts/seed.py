"""Seed initial data: organization, super admin user and example categories.
...
def main() -> None:
    ...
    with app.app_context():
        ...
        if not admin_cpf or not validate_cpf(admin_cpf):
            print(
                "ERRO: defina SEED_ADMIN_CPF com um CPF válido no .env antes de rodar o seed."
            )
            sys.exit(1)
        if not admin_password:
            print("ERRO: defina SEED_ADMIN_PASSWORD no .env.")
            sys.exit(1)
        ...
        if not org:
            ...
            print(f"Organização já existia: {org.name}")
        ...
        if not user:
            ...
            db.session.add(user)
            db.session.flush()
            print(f"Usuário super admin criado: {user.full_name} ({user.cpf})")
        else:
            print(f"Usuário com CPF {user.cpf} já existia.")
        ...
        if not existing:
            ...
            print("Vínculo ORG_SUPER_ADMIN criado.")

        for name in ("Psicólogo", "Assistente Social", "Neuropedagogo"):
            if not (
                ...
                db.session.add(
                    ProfessionalCategory(organization_id=org.id, name=name)
                )
